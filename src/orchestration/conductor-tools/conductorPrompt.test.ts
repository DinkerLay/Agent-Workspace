...
describe("Conductor system prompt", () => {
  it("describes Conductor as task owner and workers as provider-native sessions", () => {
    ...
    expect(prompt).toContain("Available Agent Workspace tools: call_session, call_sessions, read_task_state, read_session, claim_task_completion.");
    expect(prompt).toContain("Use call_session for one session-level work contract");
    ...
    expect(prompt).toContain("After a successful dispatch, you may issue more independent work");
    ...
    expect(prompt).toContain("Use claim_task_completion only after durable worker evidence and the declared artifact rule support final delivery");
    expect(prompt).toContain("the user checks the actual artifact and marks the Task achieved");
