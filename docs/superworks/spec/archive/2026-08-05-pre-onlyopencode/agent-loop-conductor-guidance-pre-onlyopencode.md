...
If a user challenges a completed search or a delivered result, the Task/Run
service—not the Conductor—applies the achieved-Task continuation policy defined
in `task-template-runtime-model.md`. The Conductor receives the resulting
user input only after that service has selected and recorded the valid
continuation identity. It may then open new native Session Agent work; the user
does not need to find or directly operate an old Searcher terminal.
